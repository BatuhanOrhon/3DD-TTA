...
def tta_gsd_reconstruct(x, lion, graph_spectral_module, steps_back_local, gamma, eta, p, loss_weights=None, total=100, use_static_style=False, dynamic_graph=False, graph_update_interval=1):
    ...
    for i, t in enumerate(timesteps_local):
        ...
        # Project chamfer onto spectral if they conflict, prioritizing topology conservation
        g_chamfer_latent_proj = project_conflicting_gradients(g_chamfer_latent, g_spectral_latent)
        g_chamfer_style_proj = project_conflicting_gradients(g_chamfer_style, g_spectral_style)

        # Aggregate conflict-free gradients
        final_grad_latent = 0
        if g_spectral_latent is not None:
            final_grad_latent = final_grad_latent + g_spectral_latent
        if g_chamfer_latent_proj is not None:
            final_grad_latent = final_grad_latent + g_chamfer_latent_proj

        final_grad_style = 0
        if g_spectral_style is not None:
            final_grad_style = final_grad_style + g_spectral_style
