...
def notes_for_run(args: SimpleNamespace) -> str:
    ...
    if args.method == SCD_NORMALIZATION_METHOD:
        return (
            "# SCD normalization control\n\n"
            "Purpose: test the Eq. 11 point-count normalization of the existing "
            "Selective Chamfer Distance while retaining the original-style "
            "shape_latent decoder, raw LION weights, LION eval mode, and the "
            "existing scheduler/gamma/eta/lambda settings. This is a one-factor "
            "reproduction control, not a new TTA method.\n"
            ...
            "seed 0/1/2, EMA disabled. Each trajectory uses one VAE encode and "
...
def parse_arguments(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    if args.method == SCD_NORMALIZATION_METHOD:
        if args.dataset_name != "modelnet-c" or args.severity != 5:
            parser.error("scd_normalization_control is locked to ModelNet40-C severity 5.")
        if args.batch_size != 32 or args.seed not in (0, 1, 2) or args.max_batches != 0:
            parser.error("scd_normalization_control is locked to batch 32, seed 0/1/2, and complete evaluation.")
        if (args.corruptions != list(SCD_NORMALIZATION_PILOT_CORRUPTIONS)
                and args.corruptions != list(CORRUPTIONS)):
            parser.error(
                "scd_normalization_control requires the Gaussian/Impulse pilot "
                "or the complete canonical all-15 corruption set.")
        if args.lion_ema_mode:
            parser.error("scd_normalization_control requires raw LION weights; EMA is disabled.")
        if (args.gamma, args.eta, args.lambdaa) != (0.01, 0.01, 0.95):
