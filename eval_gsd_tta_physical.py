...
from tta_gsd_physical import tta_gsd_reconstruct_physical
...
def parse_arguments():
    ...
    parser.add_argument('--csv_name', type=str, default="eval_results_physical.csv")
...
def configure_model(args):
    ...
    # Note: use_4d_gft is irrelevant for Physical graph calculation, but we keep the parameter initialized
    graph_spectral_module = GraphSpectralDNA(k=10, delta=0.1, gamma=0.6, M=args.M, M_mid=args.M_mid, M_high=args.M_high, use_4d_gft=False, device='cuda')
...
def process_batches(dataloader, base_model, diff_model, graph_spectral_module, args, num_steps):
    ...
    for data, label in tqdm(dataloader, desc="Processing Batches"):
        ...
        pred_points, _ = tta_gsd_reconstruct_physical(
            x=data_sample, 
            lion=diff_model, 
...
def main():
    ...
    for corruption in noises:
        ...
        print(f"\nEvaluating Corruption: {corruption} (Physical-to-Latent Architecture)")
