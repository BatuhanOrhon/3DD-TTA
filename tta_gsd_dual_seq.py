...
def tta_gsd_reconstruct_seq(x, lion, graph_spectral_module, steps_back_local, steps_back_global, gamma, eta, p, loss_weights=None, total=100, use_static_style=False):
    """
    Test-Time Adaptation (TTA) using a Sequential Dual Diffusion approach.
    Phase 1: Unconditional Global Prior diffusion to clean shape_latent.
    Phase 2: Local Prior diffusion with GSD guidance.
    ...
    - gamma: Step size for updating style_cond during Phase 2.
    - eta: Step size for updating latent points during Phase 2.
    - p: Proportion of points to consider in Chamfer Distance.
    - loss_weights: Dictionary of loss weights.
    - total: Total number of diffusion steps (default: 100).
    - use_static_style: If true, stops style_cond from updating during Phase 2.
    """
    ...
    # PHASE 1: GLOBAL PRIOR DIFFUSION (Unconditional)
    if steps_back_global > 0:
        ...
        for i, t in enumerate(timesteps_global):
            ...
            noisy_z = scheduler.step(noise_pred_global, t, noisy_z).prev_sample
        clean_z = noisy_z
    else:
        clean_z = shape_latent

    # Global style conditioning from the cleaned z
    style_cond = vae.global2style(clean_z)
    
    # PHASE 2: LOCAL PRIOR DIFFUSION WITH GUIDANCE
    if steps_back_local > 0:
        ...
        noisy_latent_point = latent_point
    ...
    # Final Decoding
    final_style = clean_z if use_static_style else style_cond
