...
Current next action: the source-only severity 1--5 probe is complete and shows a 22.1907 pp severity-1-to-5 drop, but no tested severity matches the paper's 57.6% source row. The preprocessing identity control is validated at 55.0243% (+1.3344 pp versus source-only), while pure VAE encode/decode is 54.7947% (+1.1048 pp versus source-only and -0.2296 pp versus identity). Under this one-seed control, most of the modest positive delta is therefore localized to preprocessing; VAE reconstruction adds no net gain. Both opt-in seed-stability diagnostics are implemented and await Colab seeds 1 and 2: preprocessing identity isolates interpolation randomness, while pure VAE additionally includes VAE latent sampling. The remaining source-to-TTA gap is not yet a causal diffusion/guidance estimate because the context TTA run is from a different commit and random draw. EMA, GSD/PxP and other datasets remain parked.
